...
function unzipToDirectory(
    ...
    mkdirp(directoryPath, (err: Error) => {
        ...
        yauzl.open(zipPath, { lazyEntries: true }, function (
            err: Error,
            zipfile: any
        ) {
            if (err) throw err;
                ...
                if (/\/$/.test(entry.fileName)) {
                    ...
                    zipfile.openReadStream(entry, function (
                        ...
                        mkdirp(path.dirname(entry.fileName), function (
                            ...
                            if (err) throw err;
                            readStream.pipe(
                                fs.createWriteStream(entry.fileName)
                            );
...
describe("Hashing utility", () => {
    ...
    const TEST_ARCHIVE_FILE_PATH = path.join(
        __dirname,
        "resources",
        "test.zip"
    );
    const TEST_ZIP_HASH =
        "540fed8df3553079e81d1353c5cc4e3cac7db9aea647a85d550f646e8620c317";
    const TEST_ZIP_MANIFEST_HASH =
        "9e0499ce7df5c04cb304c9deed684dc137fc603cb484a5b027478143c595d80b";
    const HASH_B =
    ...
    const IGNORED_METADATA_ARCHIVE_FILE_PATH = path.join(
        __dirname,
        "resources",
        "ignoredMetadata.zip"
    );
    ...
    it("generates a package hash from file", (done) => {
        hashUtils
            .hashFile(TEST_ARCHIVE_FILE_PATH)
    ...
    it("generates a package manifest for a directory", (done) => {
        ...
        unzipToDirectory(TEST_ARCHIVE_FILE_PATH, directory)
            .then(() => {
                return hashUtils.generatePackageManifestFromDirectory(
    ...
    it("generates a package manifest for an archive with ignorable metadata", (done) => {
        hashUtils
            .generatePackageManifestFromZip(IGNORED_METADATA_ARCHIVE_FILE_PATH)
            .done((manifest: PackageManifest): void => {
