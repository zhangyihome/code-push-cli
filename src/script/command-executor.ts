...
export var log = (message: string | chalk.ChalkChain): void =>
    console.log(message);
    ...
    return new Promise<boolean>((resolve): void => {
        ...
        prompt.get(
            ...
            (err: any, result: any): void => {
                var accepted =
                    result.response && result.response.toLowerCase() === "y";
                var rejected =
                    !result.response || result.response.toLowerCase() === "n";
                ...
                if (accepted) {
                    ...
                    if (!rejected) {
                        console.log(
                            'Invalid response: "' + result.response + '"'
                        );
                    }
                    resolve(false);
                }
        ...
        (wasConfirmed: boolean): Promise<void> => {
            if (wasConfirmed) {
                return sdk.removeAccessKey(command.accessKey).then((): void => {
                    log(
                        `Successfully removed the "${command.accessKey}" access key.`
                    );
                });
            }
...
function appTransfer(command: cli.IAppTransferCommand): Promise<void> {
    ...
    return confirm().then(
        (wasConfirmed: boolean): Promise<void> => {
            if (wasConfirmed) {
                return sdk
                    .transferApp(command.appName, command.email)
                    .then((): void => {
                        log(
...
function addCollaborator(command: cli.ICollaboratorAddCommand): Promise<void> {
    ...
    return sdk
        .addCollaborator(command.appName, command.email)
        .then((): void => {
            log(
    ...
    return Promise.all<any>([
        ...
        ([account, deploymentHistory, metrics]: [
            ...
            printDeploymentHistory(
                command,
                <PackageWithMetrics[]>deploymentHistory,
                account.email
            );
        }
    );
...
function deserializeConnectionInfo(): ILoginConnectionInfo {
    try {
        ...
        var connectionInfo:
            | ILegacyLoginConnectionInfo
            | ILoginConnectionInfo = JSON.parse(savedConnection);
        ...
        if ((<ILegacyLoginConnectionInfo>connectionInfo).accessKeyName) {
            connectionInfo = <ILoginConnectionInfo>{
                accessKey: (<ILegacyLoginConnectionInfo>connectionInfo)
                    .accessKeyName,
            };
        }
    ...
    return Promise.resolve().then(() => {
        switch (command.type) {
            ...
            case cli.CommandType.register:
                if (connectionInfo) {
                    throw new Error(
                        "You are already logged in from this machine."
                    );
                }
                break;
        ...
        switch (command.type) {
            ...
            case cli.CommandType.collaboratorRemove:
                return removeCollaborator(
                    <cli.ICollaboratorRemoveCommand>command
                );
            ...
            case cli.CommandType.deploymentHistory:
                return deploymentHistory(
                    <cli.IDeploymentHistoryCommand>command
                );
                ...
                if (!connectionInfo.preserveAccessKeyOnLogout) {
                    ...
                    return sdk
                        ...
                        .catch((error: CodePushError) => {
                            ...
                            if (
                                error.statusCode !==
                                    AccountManager.ERROR_UNAUTHORIZED &&
                                error.statusCode !==
                                    AccountManager.ERROR_NOT_FOUND
                            ) {
                                throw error;
                            }
                        });
        ...
        printTable(headers, (dataSource: any[]) => {
            deploymentHistory.forEach((packageObject: Package) => {
                ...
                if (packageObject.releaseMethod === "Promote") {
                    ...
                    var labelNumber: number = parseInt(
                        packageObject.label.substring(1)
                    );
...
function applyChalkSkippingLineBreaks(
    applyString: string,
    chalkMethod: (string: string) => chalk.ChalkChain
): string {
...
function getPackageMetricsString(obj: Package): string {
    ...
    if (!packageObject || !packageObject.metrics) {
        return (
            chalk.magenta("No installs recorded").toString() +
            (rolloutString || "")
        );
    }

    var activePercent: number = packageObject.metrics.totalActive
        ? (packageObject.metrics.active / packageObject.metrics.totalActive) *
          100
        : 0.0;
...
function getReactNativeProjectAppVersion(
    ...
    if (command.platform === "ios") {
                ...
                for (var i = 0; i < knownLocations.length; i++) {
                    ...
                    if (fileExists(propertiesFile)) {
                        const propertiesContent: string = fs
                            ...
                            .toString();
                        try {
                            const parsedProperties: any = properties.parse(
                                propertiesContent
                            );
        ...
        return parseXml(appxManifestContents)
            ...
            .then((parsedAppxManifest: any) => {
                try {
                    return parsedAppxManifest.Package.Identity[0][
                        "$"
        ...
        printTable(
            ...
            (dataSource: any[]): void => {
                ...
                keys.forEach(
                    (key: AccessKey) =>
                        !isExpired(key) &&
                        dataSource.push(keyToTableRow(key, /*dim*/ false))
                );
                keys.forEach(
                    (key: AccessKey) =>
                        isExpired(key) &&
                        dataSource.push(keyToTableRow(key, /*dim*/ true))
                );
            }
        );
    }
    ...
    if (format === "json") {
        ...
        printTable(["Machine", "Logged in"], (dataSource: any[]): void => {
            sessions.forEach((session: Session) =>
                dataSource.push([
                    session.machineName,
                    formatDate(session.loggedInTime),
                ])
            );
        });
    }
...
function patch(command: cli.IPatchCommand): Promise<void> {
    ...
    throw new Error(
        "At least one property must be specified to patch a release."
    );
}
...
export var release = (command: cli.IReleaseCommand): Promise<void> => {
    ...
    return Promise.resolve().then(() => {
        ...
        var releaseHooksPromise = hooks.reduce(
            (
                accumulatedPromise: Q.Promise<cli.IReleaseCommand>,
...
export var releaseCordova = (
    ...
    return validateDeployment(command.appName, command.deploymentName)
        .then((): any => {
            ...
            var platformFolder: string = path.join(
                projectRoot,
                "platforms",
                platform
            );
            ...
            log(
                chalk.cyan(
                    `Running "${cordovaCLI} ${cordovaCommand}" command:\n`
                )
            );
            try {
                execSync(
        ...
        validateDeployment(command.appName, command.deploymentName)
            .then((): any => {
                ...
                switch (platform) {
                    ...
                    default:
                        throw new Error(
                            'Platform must be "android", "ios", or "windows".'
                        );
                }

                try {
                    var projectPackageJson: any = require(path.join(
                        process.cwd(),
                        "package.json"
                    ));
...
export var runReactNativeBundleCommand = (
    bundleName: string,
    development: boolean,
    entryFile: string,
    outputFolder: string,
...
function sessionRemove(command: cli.ISessionRemoveCommand): Promise<void> {
    if (os.hostname() === command.machineName) {
        throw new Error(
            "Cannot remove the current login session via this command. Please run 'code-push logout' instead."
        );
    } else {
        return confirm().then(
            (wasConfirmed: boolean): Promise<void> => {
                if (wasConfirmed) {
                    return sdk
                        .removeSession(command.machineName)
                        .then((): void => {
                            log(
                                `Successfully removed the login session for "${command.machineName}".`
                            );
                        });
                }
