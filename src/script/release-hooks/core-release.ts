...
var coreReleaseHook: cli.ReleaseHook = (
    ...
    return Q(<void>null)
            ...
            return Promise<string>(
                (
                    ...
                    var packagePath: string = path.join(
                        process.cwd(),
                        CommonUtils.generateRandomFilename(15) + ".zip"
                    );
                    var zipFile = new yazl.ZipFile();
                    var writeStream: fs.WriteStream = fs.createWriteStream(
                        packagePath
                    );
        ...
        .then(
            (packagePath: string): Promise<cli.IReleaseCommand> => {
                ...
                return sdk
                    ...
                    .then(
                        (isAuth: boolean): Promise<Package> => {
                            return sdk.release(
                    ...
                    .then((): void => {
                        log(
                            `Successfully released an update containing the "${originalCommand.package}" ` +
                                `${
                                    fs
                                        .lstatSync(originalCommand.package)
                                        .isDirectory()
                                        ? "directory"
                                        : "file"
                                }` +
