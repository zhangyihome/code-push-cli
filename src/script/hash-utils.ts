...
export function generatePackageManifestFromZip(
    filePath: string
    ...
    yauzl.open(
        filePath,
        { lazyEntries: true },
        (error?: any, openedZipFile?: any): void => {
            if (error) {
            ...
            zipFile
                ...
                .on("entry", (entry: any): void => {
                    var fileName: string = PackageManifest.normalizePath(
                        entry.fileName
                    );
                    ...
                    zipFile.openReadStream(
                        entry,
                        (error?: any, readStream?: stream.Readable): void => {
                            if (error) {
                                reject(error);
                                return;
                            }
